...
async def main():
    ...
    try:
        ...
        async for message in client.iter_messages(source_entity, reverse=True, min_id=last_sent_id):
            ...
            while retries > 0:
                try:
                    # Encaminha a mensagem respeitando integralmente a mídia e formatação original sem citar autor/fonte
                    await client.forward_messages(dest_entity, message, drop_author=True)
                    success = True
                    break
